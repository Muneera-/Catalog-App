...
# cheack thread is set to false to avoid having multiple connections
engine = create_engine(
    'sqlite:///bookcatalogswithusers.db?check_same_thread=False')
...
def editItem(categories_id, items_id):
    ...
    if request.method == 'POST':
        ...
        session.commit()
        flash("Updated book item successfully!")
...
def deleteItem(categories_id, items_id):
    ...
    if itemToDelete.user_id != login_session['user_id']:
        ...
        {alert('You are not authorized "\
            "to delete this item. Please create \
    ...
    if request.method == 'POST':
        ...
        session.commit()
        flash("Deleted book item successfully!")
